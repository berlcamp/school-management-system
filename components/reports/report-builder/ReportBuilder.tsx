...
/**
 * Custom Report Builder — the escape hatch beside the fixed reports.
 *
 ...
 *
 * ONE COMPONENT, TWO SCOPES. The division office reaches it at
 * /division/reports/builder and may report on every school at once; a school
 * reaches it under /school-reports and is PINNED to their own. The pinning is
 * not this component's doing and must not be trusted as if it were:
 * `can_run_division_report` (166) refuses a school user any scope but their
 * own school, and the NULL scope outright. What the caller passes in `schoolId`
 * only decides what is ASKED for.
 */
...
import { SavedReportBar, SaveDetails } from "./SavedReportBar";
...
export interface ReportBuilderProps {
  /** The scope to report on. null = every school (division only). */
  schoolId: number | null;
  /** How that scope reads in the summary line: "All Schools", or a name. */
  scopeLabel: string;
  /** The division page's school picker; a school user gets none. */
...
export function ReportBuilder({
  ...
  // A division user changing schools in the bar abandons the result: it was
  // another school's, and leaving it on screen under a new heading is how a
  // figure gets read against the wrong school.
  useEffect(() => {
    setSpec(null);
    setRows([]);
    setTotal(0);
  ...
  const handleLoadDefinition = (definition: ReportDefinition | null) => {
    ...
    // The saved scope is deliberately NOT applied: a school user is pinned to
    // their own school and a definition must never move them off it. For the
    // division office the scope stays whatever the bar says, which is the one
    // control always in view.
  ...
  const outputDisabled = !spec || total === 0 || exporting;
        ...
        <CardContent className="p-4">
          <div className="flex flex-wrap items-end gap-3">
            ...
            <div className="flex items-end gap-2">
              ...
              <Button size="sm" onClick={handlePrint} disabled={outputDisabled}>
          ...
          {catalogueLoading ? (
            ...
            <>
              ...
              <SavedReportBar
                ...
                allowDivisionSharing={allowDivisionSharing}
