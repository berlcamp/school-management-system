...
- Check that page-specific components are co-located under `components/` subfolders (per CLAUDE.md convention)
...
**Anti-patterns:**
- `any` TypeScript types (violates CLAUDE.md) — `grep -rn ": any" app/ lib/ types/`
...
- Missing `isMounted` flag checks (per CLAUDE.md convention) — can cause setState after unmount warnings
...
- When in doubt about a DepEd workflow, refer to CLAUDE.md which documents the authoritative system design.
