...
export default function Page() {
  ...
  const [filter, setFilter] = useState<RmaFilter>({ keyword: "" });

  const dispatch = useAppDispatch();
  const list = useAppSelector((state) => state.list.value);
  const filterKeywordRef = useRef(filter.keyword);

  const handleFilterChange = useCallback((newFilter: RmaFilter) => {
    setFilter(newFilter);
      ...
      <div className="app__content">
        {loading ? (
          ...
          <div className="app__empty_state">
            ...
            <p className="app__empty_state_title">No RMA materials found</p>
            <p className="app__empty_state_description">
              {filter.keyword || filter.grade_level !== undefined
                ? "Try adjusting your search criteria"
                : "Get started by adding an RMA material for a grade level"}
            </p>
          </div>
        ) : (
          <List schoolId={null} />
