...
| **Kindergarten Progress Report** | `teacher/kinder-progress/`, `lib/pdf/generateKinderProgressReport.ts`, `lib/constants/kinderProgress.ts`, migration 172 | The parent-facing Kindergarten card: BG/DV/CO per Kindergarten Curriculum Guide competency per ten-week **term** (T1/T2/T3), plus the adviser's per-term comments. **Not the ECCD checklist** (`teacher/eccd/`) — a different instrument on a different scale and cadence; schools file both. Printed as a two-page facsimile of the issued SDO form: competency grid, teacher's comments with parent signature lines, attendance record, rating legend, Certificate of Transfer |
...
| 172 | Kindergarten Progress Report — the card the adviser hands the parent, rating every Kindergarten Curriculum Guide competency BG (Beginning) / DV (Developing) / CO (Consistent) every ten weeks. **Deliberately not the ECCD checklist (047/059).** The two instruments answer different questions and schools file both: ECCD is 7 development domains, a 0/1 checkbox, two semesters and a raw→scale score against a national norm; this is 4 curriculum domains, a lettered scale, three terms and no scoring at all — the rating *is* the report. Reusing `sms_eccd_*` would have meant widening its rating CHECK from `(0,1)` to letters and its period CHECK to carry terms, silently re-interpreting every ECCD row already encoded, so: four new tables, nothing existing touched. `sms_kinder_progress_domains` carries `print_column` (1 = left half of the printed grid, 2 = right, because domain IV alone is longer than I–III together) so the layout survives a domain being added; `sms_kinder_progress_competencies` interleaves the strand titles among the items as rows carrying `is_heading` rather than putting them in a grouping table — domain IV nests two levels deep (Reading → Letter Knowledge → items) and one ordered list reproduces the printed page exactly, so a DepEd revision that promotes or demotes a strand is a row edit. `rating` is CHECK-constrained per 133/153 rather than free TEXT per 119/132: the three letters are the instrument, they are printed back verbatim, and the legend on page 2 enumerates exactly these three. **Clearing a rating DELETEs the row** — an unrated competency prints a blank cell, which is what a card printed mid-year is supposed to look like, and a NULL would have to be excluded at every read. Remarks are their own table because the comment belongs to the learner's *term*, not to any competency, and the form prints a parent's signature line under each of the three. **Two items are transcribed with their issued typos** ("Recognizes non-doable words…" for *decodable*; "Matches letters and their corresponding sounds" listed twice under Letter Sound Relationship) per the 137/154 rule that a figure taken off a DepEd form is printed back verbatim — a school comparing screen to paper must find the same lines; fixing either is an UPDATE on one row once the division confirms the intent. Attendance on the printed card is **not stored**: it is `sms_attendance` resolved through 125's school calendar on exactly the rules the attendance grid, SF2 and the report card already use. Seed is `ON CONFLICT (code) DO NOTHING`, so re-applying cannot duplicate an item nor overwrite a description the division has since edited. RLS = authenticated with app-layer roster scoping, matching 105/119/121 |
...
12. **`sms_users.type` is the *active* role, `sms_user_roles` is the *permitted set*** (migration 163) — a person may hold several jobs at a school but works as one at a time, which is what every `type` check in SQL and in the client reads. Never widen a policy or a guard to "any assigned role"; change the active role through `sms_switch_active_role` (or `sms_switch_active_context`, which moves school and role together) instead. A school head edits the set for their own school's staff at `/staff`; the division office at `/division/users`.
13. **`sms_users.school_id` is the *active* school, `sms_user_schools` is the *permitted set*** (migration 134) — a user may be assigned to several schools but is only ever in one at a time, which is what every school-scoped query and RLS policy reads. Never widen a policy to "any assigned school"; change the active school through `sms_switch_active_school` instead. Assignments are edited only at `/division/users`.
